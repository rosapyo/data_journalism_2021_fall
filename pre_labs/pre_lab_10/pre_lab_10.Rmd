...
**Task** Click on Refine. The first option we want to change is the Number Format, because we have currency figures to display and we want to make it as easy as possible for readers to understand what we're displaying. Datawrapper has an excellent reference for its [custom formats](https://academy.datawrapper.de/article/207-custom-number-formats-that-you-can-display-in-datawrappef l) that you can consult. Let's choose `$0.[00]a`, which adds a dollar sign and abbreviates larger amounts:
...
https://app.datawrapper.de/chart/W8tFn/publish#export-image 
...
It is taking all the md counties and giving number of PPP per county.
...
**Answer** Describe what the code below is doing and the output.
taking busnisses and grouping by county name 
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
counts the ppp loans per county and gives the fip code for the county
...
It only shows maryland and zooms in. 
...
Shows the county name and number of loans
...
https://app.datawrapper.de/map/5Oazs/publish
