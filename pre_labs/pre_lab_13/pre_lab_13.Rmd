...
**Answer**Shows in a table a states name and which president they primarily voted for as well as the amount of ppe loans they received 
```{r}
...
**Answer** There is a linear line that shows a correlation.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The line is more flat and seems to be less correlated.
```{r}
...
**Answer** There is a 	0.52 correlation between trump won states and businesses receiving ppe loans. 
```{r}
reveal_data %>%
...
**Answer** It gave the saame correlaation of around 0.52
```{r} 
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The correlation is then 0.22
```{r}
...
**Answer** The mean ppp amount per employee is about the same for trump and clinton. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** There is aan almost ten peercent difference in the means between clinton and trumpx
```{r}
